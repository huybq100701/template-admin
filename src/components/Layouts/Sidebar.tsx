...
import { getAuthorizations } from '../../services/tab.service';
                ...
                <div className="bg-white dark:bg-black h-full">
                    <div className="flex justify-between items-center px-4 py-3">
                        <NavLink to="/" className="main-logo flex items-center shrink-0">
                            <img className="w-8 ml-[5px] flex-none" src="/app-icon.jpg" alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5 font-semibold align-middle lg:inline dark:text-white-light">{t('EVENTIC')}</span>
                        </NavLink>
                    ...
                    <PerfectScrollbar className="h-[calc(100vh-80px)] relative">
                        <ul className="relative font-semibold space-y-0.5 p-4 py-0">
                            {tabs?.map((tab: any) => {
                                if (tab?.tab_name === 'Dashboard') {
                                    return (
                                        <div>
                                            <li className="nav-item">
                                                <ul>
                                                    <li className="nav-item">
                                                        <NavLink to={tab?.tab_path} className="group">
                                                            <div className="flex items-center">
                                                                <Icon icon={tab?.tab_icon} className="group-hover:!text-primary shrink-0" />
                                                                <span className="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{tab?.tab_name}</span>
                                                            </div>
                                                        </NavLink>
                                                    </li>
                                                </ul>
                                    ...
                                    return (
                                        <div>
                                            <h2 className="py-3 px-7 flex items-center uppercase font-extrabold bg-white-light/30 dark:bg-dark dark:bg-opacity-[0.08] -mx-4 mb-1">
                                                ...
                                                {themeConfig.menu !== 'vertical' || <span>{tab?.tab_name}</span>}
                                            </h2>
                                            <li className="nav-item">
                                                <ul>
                                                    {tab?.child_tabs?.map((child_tab: any) => (
                                                        <li className="nav-item">
                                                            <NavLink to={`${tab?.tab_path}${child_tab?.tab_path}`} className="group">
                                                                <div className="flex items-center">
                                                                    <Icon icon={child_tab?.tab_icon} className="group-hover:!text-primary shrink-0" />
                                                                    <span className="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{child_tab?.tab_name}</span>
                                                                </div>
                                                            </NavLink>
                                                        </li>
                                                    ))}
